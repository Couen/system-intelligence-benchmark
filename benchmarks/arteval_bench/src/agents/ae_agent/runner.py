#!/usr/bin/env python3
"""AE Agent runner for ArtEvalBench (system-intelligence-benchmark).

Uses Claude Agent SDK to run artifact evaluation tasks inside the benchmark container.
Contract: artifact at /repo, this agent at /agent; task passed as CLI arg or path to file.
"""
...
# Default 48h in ms when not provided by env
...
def main():
    """Entry point. Second arg can be task text or path to a file (avoids shell quoting)."""
    if len(sys.argv) != 3:
        ...
        print("         python3 runner.py claude-sonnet-4-5-20250929 /agent/current_task.txt", file=sys.stderr)
        sys.exit(1)
